...
test('Solo practice reads a tossup, buzz and answer', async ({ page }) => {
  ...
  const reader = page.locator('.solo__reading');
  await expect(reader).toBeVisible({ timeout: 20_000 });

  // let a few words reveal, then buzz
  await page.waitForTimeout(2500);
  await page.locator('.solo__buzz').click();

  // type an answer and submit
  const input = page.locator('.solo__input');
  ...
  await expect(page.locator('.solo__result')).toBeVisible({ timeout: 15_000 });
